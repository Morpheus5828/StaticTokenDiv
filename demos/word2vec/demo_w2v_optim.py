...
def run(optim_choice: bool, nb_iteration: int = 5, k_max: int = 10, k_min: int = 1):
    scores = []
    for k in range(k_min, k_max):
        ...
        for i in range(nb_iteration):
            ...
            w2v.word2vec(
                ...
                optmim_random_choice=optim_choice,
...
times = run(optim_choice=True)
plt.plot(range(len(times)), times, c="blue", label="with negative sampling")
scores = run(optim_choice=False)
plt.plot(range(len(scores)), scores, c="red", label="without negative sampling")
plt.xlabel("k")
plt.ylabel("Scores")
